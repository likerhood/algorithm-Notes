...
栈的特点是后进先出，那么栈顶的元素肯定是最靠近当前序列中的当前元素的。不管是需要判断当前元素的左边还是右边的最近的大于或者小于的元素，我们只需要记住这一点：**==栈顶的元素肯定是最靠近当前序列中的当前元素的==**。

理解这一点就很好办了。如果需要得出当前序列中某一元素的左边最近的小于他的元素，**我们维护栈的元素的单调性，保证栈底到栈顶的元素是单调递增的。** ==如果栈顶的元素小于当前元素，那么就是左边最近的小于当前元素的值。== 如果栈顶元素大于当前元素，那么弹出，直到遇到满足条件的元素。如果栈为空，那么说明该元素左边没有比他小的元素。最终需要将当前元素压入栈，保证是离下一个判断的元素最近的元素。

如果需要得出当前序列中某一元素的右边最近的小于他的元素，那么栈里面的元素就是存储当前序列的左边的元素。**这里不太一样的是每次拿栈顶元素和序列下一个元素作比较，维护栈中元素是单调递增的。** 当栈顶元素大于下一个元素时，说明栈顶元素右边第一个小于他的元素就是当前比较的元素。此时记录并弹出栈。直到栈顶元素和当前比较元素小，停止，并将该元素压入栈。当序列所有元素都遍历完，最后还留在栈里面的元素在序列中他右边是没有比他小的元素了。
...
**Trie功能**
根据Trie的概念，可知它的本质是将很多字符串拆分成单个字符的形式，以树的形式存储起来，这里的查询方式类似字典。应用如下：
